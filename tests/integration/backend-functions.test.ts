...
describe("주문 — 상품 상태/권한/상태전이", () => {
  ...
  it("in-flight 발주(sentAt 있고 주문번호 없음)도 수동 완료 불가 (TOCTOU 부분환불 유실 방지)", async () => {
    const admin = await makeUser(0, "ADMIN");
    const u = await makeUser();
    const p = await makeProduct();
    const o = await prisma.order.create({
      data: {
        orderNo: `${PREFIX}_inflight_${Math.random().toString(36).slice(2, 8)}`,
        userId: u.id,
        status: "PROCESSING",
        totalAmount: 100,
        items: {
          create: {
            productId: p.id, productName: "t", unitPrice: 100, quantity: 1, subtotal: 100,
            // 발주 시도됨(sentAt) 이나 아직 도매 주문번호 없음 = addOrder 대기 중
            sentAt: new Date(), providerOrderId: null,
          },
        },
      },
    });
    asAdmin(admin);
    const r = await setOrderStatus(o.id, "COMPLETED");
    expect(r.ok).toBe(false); // sentAt 기준이라 in-flight도 차단
    expect((await prisma.order.findUnique({ where: { id: o.id } }))?.status).toBe("PROCESSING");
  });
});
...
describe("충전 — 입력 검증", () => {
  ...
  it("멱등성: 같은 clientKey 재제출은 신청 1건만 생성(따닥→이중신청 방지)", async () => {
    await prisma.setting.deleteMany({ where: { key: "vat_rate" } });
    const u = await makeUser();
    asUser(u);
    const key = `ck_${Date.now().toString(36)}`;
    const r1 = await createChargeRequest({ amount: 10_000, depositorName: "홍길동", clientKey: key });
    const r2 = await createChargeRequest({ amount: 10_000, depositorName: "홍길동", clientKey: key });
    expect(r1.ok && r2.ok).toBe(true);
    expect(r1.id).toBe(r2.id); // 같은 신청 반환
    const count = await prisma.chargeRequest.count({ where: { clientKey: key } });
    expect(count).toBe(1); // DB엔 1건만
  });
});
...
describe("충전 — 입금확인/부분입금/벌크 + 감사 로그", () => {
  ...
  it("벌크 200건 초과는 조용히 자르지 않고 거부(미처리분 오인 방지)", async () => {
    const admin = await makeUser(0, "ADMIN");
    asAdmin(admin);
...
describe("rate limit — DB 공유 카운터 (서버리스 다중 인스턴스 대응)", () => {
  ...
  it("초장문 키도 fail-open 없이 정상 제한(해시 고정길이) — 리미터 우회 차단", async () => {
    // btree 인덱스 한도를 넘길 만큼 긴 키(공격자 제어 username 모사) → 해시로 고정길이화되어
    // INSERT 실패(→catch→fail-open) 없이 정상 카운트되어야 한다.
    const longKey = "a".repeat(5000) + Date.now();
    expect(await rateLimit("rllong", { max: 1, windowMs: 60_000, key: longKey })).toBe(true);
    expect(await rateLimit("rllong", { max: 1, windowMs: 60_000, key: longKey })).toBe(false); // 우회 안 됨
  });
});
